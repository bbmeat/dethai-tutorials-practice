#!/usr/bin/env python3
"""
此示例对StereoDepth的视差输出进行编码。请注意，您不应启用亚像素模式，因为VideoEncoder不支持 UINT16 。

按 Ctrl+C 将停止录制，然后使用 ffmpeg 将其转换为 mp4 以使其可播放。请注意，需要安装并运行 ffmpeg 才能成功转换为 mp4。

请注意，此示例将编码的视频保存到您的主机存储中。因此，如果您让它继续运行，您可能会填满主机上的存储空间。
"""
